...
仍然假设映射到新坐标空间中的单位正交基为$\vec{e}_1,\vec{e}_2,\cdots,\vec{e}_d$，均值矢量为$\vec{\mu}$，假设要降维到$d'$维，则可令$\mathbf E=\left(\vec{e}_1,\cdots,\vec{e}_{d'}\right)$，则原始样本点$\vec{x}_k$在新空间中超平面上的投影是$\mathbf E^T\left(\vec{x}_k-\vec\mu\right)$，要让所有样本点的投影尽可能分开，则应该尽可能让投影的每一个维度的方差最大化，也即在新空间的$d$维特征中找到$d'$个维度使得这$d'$个维度的方差和最大，于是优化目标可写为
$$
\max_{\mathbf E} \left(\frac{1}{n}\tr\sum_{k=1}^n\mathbf E^T\left(\vec{x}_k-\vec{\mu}\right)
\left(\vec{x}_k-\vec\mu\right)^T\mathbf E\right)
$$
同样，这也是约束优化问题，约束为
$$
\mathbf E^T\mathbf E=\mathbf I
$$
假设原始样本集的标准差矢量为$\vec{\sigma}$，经过标准化处理后的样本为$\vec{\mathbf x}_{k}$，则有
$$
\vec{x}_k-\vec\mu=\vec{\sigma}\circ\vec{\mathbf x}_{k}
$$
于是优化目标可以化简为
$$
\begin{aligned}
\max_{\mathbf E}\left(\tr\left(\mathbf E^T\sum_{k=1}^n\left(\vec{\sigma}\circ\vec{\mathbf x}_k\left(\vec{\sigma}\circ\vec{\mathbf x}_k\right)^T\right)\mathbf E\right)\right) &=  \max_{\mathbf E}\left(\tr\left(\mathbf E^T\sum_{k=1}^n\left(\vec{\sigma}\circ\vec{\mathbf x}_k\left(\vec{\mathbf x}_k^T\circ\vec{\sigma}^T\right)\right)\mathbf E\right)\right)\\
&= \max_{\mathbf E}\left(\tr\left(\mathbf E^T\sum_{k=1}^n\left(\left(\vec{\sigma}_1,\cdots,\vec{\sigma}_d\right)\circ\left(\vec{\mathbf x}_k\vec{\mathbf x}_k^T\right)\circ\left(\vec{\sigma}_1,\cdots,\vec{\sigma}_d\right)^T\right)\mathbf E\right)\right) \\
&= \max_{\mathbf E}\left(\tr\left(\mathbf E^T\left(\left(\vec{\sigma}_1,\cdots,\vec{\sigma}_d\right)\circ\left(\sum_{k=1}^n\vec{\mathbf x}_k\vec{\mathbf x}_k^T\right)\circ\left(\vec{\sigma}_1,\cdots,\vec{\sigma}_d\right)^T\right)\mathbf E\right)\right) \\
&= \max_{\mathbf E}\left(\tr\left(\mathbf E^T\left(\left(\vec{\sigma}_1,\cdots,\vec{\sigma}_d\right)\circ\left(\mathbf X\mathbf X^T\right)\circ\left(\vec{\sigma}_1,\cdots,\vec{\sigma}_d\right)^T \right)\mathbf E\right)\right)
\end{aligned}
$$
其中，$\vec{\sigma}_1=\vec{\sigma}_2=\cdots=\vec{\sigma}_d=\vec{\sigma}$，$\mathbf X=\left(\vec{\mathbf x}_1,\cdots,\vec{\mathbf x}_n\right)$。
